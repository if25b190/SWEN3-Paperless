...
export async function apiFetch<T>(path: string, options: ApiOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && auth !== false && typeof window !== "undefined") {
...
export type Correspondent = { id: number; name: string; notes?: string | null };
...
export const api = {
  login: (body: { username: string; password: string }) => apiFetch<{ token: string; user: User }>("/auth/login", { method: "POST", body: JSON.stringify(body), auth: false }),
  register: (body: { username: string; email: string; password: string }) => apiFetch<User>("/users", { method: "POST", body: JSON.stringify(body), auth: false }),
  ...
  labels: (kind: "correspondents" | "document-types") => apiFetch<{ items: Label[] }>(`/${kind}`), correspondent: (id: number) => apiFetch<Correspondent>(`/correspondents/${id}`), documentType: (id: number) => apiFetch<DocumentType>(`/document-types/${id}`), createLabel: (kind: "correspondents", body: { name: string; notes?: string }) => apiFetch<Correspondent>(`/${kind}`, { method: "POST", body: JSON.stringify(body) }), createDocumentType: (body: { name: string; description?: string }) => apiFetch<DocumentType>("/document-types", { method: "POST", body: JSON.stringify(body) }), updateLabel: (kind: "correspondents", id: number, body: { name?: string; notes?: string }) => apiFetch<Correspondent>(`/${kind}/${id}`, { method: "PUT", body: JSON.stringify(body) }), updateDocumentType: (id: number, body: { name?: string; description?: string }) => apiFetch<DocumentType>(`/document-types/${id}`, { method: "PUT", body: JSON.stringify(body) }), deleteLabel: (kind: "correspondents" | "document-types", id: number) => apiDelete(`/${kind}/${id}`),
  users: () => apiFetch<{ items: User[] }>("/users"), user: (id: number) => apiFetch<User>(`/users/${id}`), createUser: (body: { username: string; email: string; password: string }) => apiFetch<User>("/users", { method: "POST", body: JSON.stringify(body), auth: false }), updateUser: (id: number, body: Partial<{ username: string; email: string; password: string }>) => apiFetch<User>(`/users/${id}`, { method: "PUT", body: JSON.stringify(body) }), deleteUser: (id: number) => apiDelete(`/users/${id}`), userTeams: (id: number) => apiFetch<{ items: { team: Team; role: Member["role"] }[] }>(`/users/${id}/teams`),
