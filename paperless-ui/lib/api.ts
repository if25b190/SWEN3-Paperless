...
export type Label = { id: number; name: string; notes?: string | null; description?: string | null };
...
export const api = {
  ...
  labels: (kind: "correspondents" | "document-types") => apiFetch<{ items: Label[] }>(`/${kind}`), createLabel: (kind: "correspondents" | "document-types", body: { name: string; notes?: string; description?: string }) => apiFetch<Label>(`/${kind}`, { method: "POST", body: JSON.stringify(body) }), updateLabel: (kind: "correspondents" | "document-types", id: number, body: { name?: string; notes?: string; description?: string }) => apiFetch<Label>(`/${kind}/${id}`, { method: "PUT", body: JSON.stringify(body) }), deleteLabel: (kind: "correspondents" | "document-types", id: number) => apiDelete(`/${kind}/${id}`),
